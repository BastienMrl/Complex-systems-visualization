...
class AntColonyParameters(SimulationParameters):
    ...
    def __init__(self, agents : list[_SimulationPair], diffusion : _SimulationPair, id_prefix : str = "default"):
        ...
        self._rules_param : list[Param] = [
            FloatParam(id_p=self._get_id_prefix() + f"dropAmount{i}", name = self._get_name_prefix() + f"Dropped Amount {i}",
                       default_value=0.02, min_value = 0.0, step = 0.05) for i in range(len(self.drop_amount))
        ]
    ...
    def get_rules_parameters(self) -> list[Param]:
        ...
        for agent in self.agents:
            ret += agent.params.get_rules_parameters()
        return ret + self.diffusion.params.get_rules_parameters()
    ...
    def init_param_value_changed(self, idx: int, param: Param) -> None:
        match(idx):
            ...
            case 1:
                ...
                self.diffusion.params.compute_init_params()
...
class AntColony(Simulation):
    ...
    def _step(self):
        ...
        for i, agent in enumerate(self.agents):
            if (self.params.drop_amount[i] == 0.):
                continue
            agent.set_grid(grid)
            agent.new_step()
...
class MultiplePhysarumSimulation(AntColony):
    ...
    agent_3 = _SimulationPair(PhysarumAgentParameters("3"), PhysarumAgentSimulation)

    diffusion = _SimulationPair(DiffusionParameters("Diffusion"), DiffusionSimulation)

    params = AntColonyParameters([agent_1, agent_2, agent_3], diffusion)
...
class PhysarumGrayScott(AntColony):
    agent = _SimulationPair(PhysarumAgentParameters("Physarum"), PhysarumAgentSimulation)
    diffusion = _SimulationPair(GrayScottParameters("Diffusion"), GrayScottSimulation)
